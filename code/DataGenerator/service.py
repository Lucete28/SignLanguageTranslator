...
model = load_model(r"C:\PlayData\TEST1991.h5")
            ...
            if len(hand_result.multi_hand_landmarks) == 2 or len(hand_result.multi_hand_landmarks) == 1:
                ...
                if not np.isnan(raw_pose_data).any(): # nan 값 확인
                    ...
                    if hand_data and pose_data:
                        data = np.concatenate((hand_data[-1], pose_data[-1]))
                        datas.append(data)
                        #print(data.shape)
                    if len(datas)>=30:
                        seq = np.array([datas[-30:]])
                        pred = model.predict(seq, verbose=0).squeeze()
                        pred_idx= int(np.argmax(pred))
                        print(pred_idx)
        cv2.imshow('img', img)
    #print(len(hand_data),len(pose_data))
